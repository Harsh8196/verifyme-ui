...
import VerifyMe from '../script/VerifyMe'
...
const ipfsBaseURL = 'https://verifyme.infura-ipfs.io/ipfs/'
...
function Issuer() {
    ...
    useEffect(() => {
        async function getStatus() {
            const status = await VerifyMe.methods.issuerAddress(accounts[0]).call()
    ...
    async function getCompanyData() {
        ...
        try {
            ...
            const issuerRequestCount = await VerifyMe.methods.issuerRequestCount().call()
            
            for (var i = 0; i < issuerRequestCount; i++) {
                const issuerRequestResult = await VerifyMe.methods.issuerRequest(i).call()
                const obj = {
    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            await ipfsClient.pin.add(metaData.path.toString())
            const result = await VerifyMe.methods.applyForRequest(metaData.path.toString()).send({ from: accounts[0] })
            // console.log(result)
