...
function CreateContract() {
    ...
    useEffect(() => {
        ...
        async function getOwnerContractStatus() {
            ...
            const result = await VerifyMe.methods.getOwnerContract(accounts[0]).call()
            if (result.length > 0) {
    ...
    const onSubmit = async (event) => {
        ...
        setLoading(false)
        try {
            const result = await VerifyMe.methods.createNewContract(index, productType, contractName).send({
                from: accounts[0]
            })
            // console.log(result)
